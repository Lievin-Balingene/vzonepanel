<?php
$web_total = is_array($data) ? count($data) : 0;
$web_ssl = 0;
$web_suspended = 0;
if (is_array($data)) {
	foreach ($data as $d) {
...
$web_active = $web_total - $web_suspended;
?>
<!-- Begin toolbar -->
<div class="toolbar">
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			<?php if ($read_only !== "true") { ?>
				<a href="/add/web/" class="button js-button-create">
					<i class="fas fa-plus"></i><?= tohtml(_("Add Web Domain")) ?>
				</a>
			<?php } ?>
			<a href="/list/web/?apps=1" class="button button-secondary">
				<i class="fas fa-cubes"></i><?= tohtml(_("Applications")) ?>
			</a>
		</div>
...
<div class="container" x-data="vzWebList">

	<div class="vz-page-hero">
		<div>
			<h1 class="vz-page-title"><?= tohtml(_("Web Hosting")) ?></h1>
			<p class="vz-page-subtitle"><?= tohtml(_("Manage domains, SSL certificates, and site applications.")) ?></p>
		</div>
		<div class="vz-stat-pills">
			<span class="vz-stat-pill"><strong><?= (int) $web_total ?></strong> <?= tohtml(_("domains")) ?></span>
			<span class="vz-stat-pill is-success"><strong><?= (int) $web_active ?></strong> <?= tohtml(_("active")) ?></span>
			<span class="vz-stat-pill is-info"><strong><?= (int) $web_ssl ?></strong> SSL</span>
			<?php if ($web_suspended > 0) { ?>
				<span class="vz-stat-pill is-danger"><strong><?= (int) $web_suspended ?></strong> <?= tohtml(_("suspended")) ?></span>
			<?php } ?>
		</div>
	</div>
	...
	<div class="units-table js-units-container vz-web-table">
...
"disabled"
  ...
  foreach ($data as $key => $value) {
  	...
  	?>
   	...
   	echo "disabled";
   } ?> js-unit vz-web-row vz-filter-row"
	...
	_("Select"),
) ?>" name="domain[]" value="<?= tohtml($key) ?>"<?= $display_mode === "disabled" ? " disabled" : "" ?>>
      	...
      	foreach ($aliases as $alias) {
      		if ($alias != "www." . $key) {
      			$alias_new[] = trim($alias);
							...
							<a href="/edit/web/?<?= tohtml(
       	http_build_query(["domain" => $key, "token" => $_SESSION["token"]]),
       ) ?>" title="<?= tohtml(_("Edit Domain")) ?>: <?= tohtml($key) ?>">
								<?= tohtml($key) ?>
							</a>
							<div class="vz-web-badges">
								...
								<?php } ?>
								<?php if (!empty($data[$key]["BACKEND"])) { ?>
									<span class="vz-badge"><?= tohtml($data[$key]["BACKEND"]) ?></span>
								<?php } ?>
							</div>
       	...
       	echo "<p class='hint u-max-width300 u-text-truncate'>" . tohtml($aliases) . "</p>";
          	...
          	http_build_query(["domain" => $key, "token" => $_SESSION["token"]]),
								...
								<li class="units-table-row-action" data-key-action="href">
									<a
										class="units-table-row-action-link"
										href="/download/site/?<?= tohtml(
          	http_build_query(["site" => $key, "token" => $_SESSION["token"]]),
          ) ?>"
							...
							<li class="units-table-row-action shortcut-s" data-key-action="js">
								<a
									...
									href="/<?= tohtml($spnd_action) ?>/web/?<?= tohtml(
	http_build_query(["domain" => $key, "token" => $_SESSION["token"]]),
) ?>"
