...
<div class="container" x-data="vzToolsHome">
	<div class="vz-dashboard-hero">
		<div>
			<h1><?= _("Dashboard") ?></h1>
			<p>
				<?= htmlspecialchars(sprintf(_("Welcome back, %s — %s"), $u["NAME"] ?? $user, $hostname)) ?>
			</p>
		</div>
 ...
 ?>
	...
	<h2 class="vz-section-title u-mt20"><?= _("Server health") ?></h2>

	<div class="vz-dash-grid" x-data="vzDashboard" data-cpu="<?= htmlspecialchars((string) ($vz_metrics["cpu_percent"] ?? "")) ?>" data-mem="<?= htmlspecialchars((string) ($vz_metrics["mem_percent"] ?? "")) ?>" data-disk="<?= htmlspecialchars((string) ($disk_pct ?? $vz_metrics["disk_root_percent"] ?? "")) ?>">
		...
		<p class="vz-section-title"><?= _("Resources") ?></p>
