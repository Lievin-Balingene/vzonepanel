...
$domain = $_GET["domain"] ?? "";
$acc_total = is_array($data) ? count($data) : 0;
$acc_suspended = 0;
if (is_array($data)) {
	foreach ($data as $d) {
		if (($d["SUSPENDED"] ?? "") === "yes") {
...
$acc_active = $acc_total - $acc_suspended;
?>
<!-- Begin toolbar -->
<div class="toolbar">
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			...
			<?php if ($read_only !== "true") { ?>
				<a href="/add/mail/?<?= tohtml(http_build_query(["domain" => $domain])) ?>" class="button js-button-create">
					<i class="fas fa-plus"></i><?= tohtml(_("Add Mail Account")) ?>
				</a>
				<a href="/edit/mail/?<?= tohtml(http_build_query(["domain" => $domain])) ?>" class="button button-secondary js-button-create">
					<i class="fas fa-pencil"></i><?= tohtml(_("Edit Mail Domain")) ?>
				</a>
			<?php } ?>
		</div>
		<div class="toolbar-right">
      	...
      	echo "active";
				...
				<?php if ($read_only !== "true") { ?>
					<form x-data x-bind="BulkEdit" action="/bulk/mail/" method="post">
						...
						<input type="hidden" value="<?= tohtml($domain) ?>" name="domain">
     	...
     	$_GET["q"] ?? "",
     ) ?>" placeholder="<?= tohtml(_("Filter accounts…")) ?>" title="<?= tohtml(_("Search")) ?>" autocomplete="off">
...
<div class="container" x-data="vzMailAccList">

	<div class="vz-page-hero">
		<div>
			<h1 class="vz-page-title"><?= tohtml(_("Email Accounts")) ?></h1>
			<p class="vz-page-subtitle"><?= tohtml(sprintf(_("Mail accounts for %s"), $domain)) ?></p>
		</div>
		<div class="vz-stat-pills">
			<span class="vz-stat-pill"><strong><?= (int) $acc_total ?></strong> <?= tohtml(_("accounts")) ?></span>
			<span class="vz-stat-pill is-success"><strong><?= (int) $acc_active ?></strong> <?= tohtml(_("active")) ?></span>
			<?php if ($acc_suspended > 0) { ?>
				<span class="vz-stat-pill is-danger"><strong><?= (int) $acc_suspended ?></strong> <?= tohtml(_("suspended")) ?></span>
			<?php } ?>
		</div>
	</div>
  ...
  foreach ($data as $key => $value) {
  	...
  	?>
   	...
   	echo "disabled";
				...
				data-name="<?= tohtml(strtolower($key . "@" . $domain)) ?>"
				x-show="isVisible($el)"
	...
	_("Select"),
) ?>" name="account[]" value="<?= tohtml($key) ?>" <?= tohtml($display_mode) ?>>
				...
				<div class="units-table-cell units-table-heading-cell u-text-bold">
					...
					<div class="vz-web-name">
						<?php if ($read_only === "true" || $data[$key]["SUSPENDED"] == "yes") { ?>
							<?= tohtml($key . "@" . $domain) ?>
						<?php } else { ?>
							<a href="/edit/mail/?<?= tohtml(
       	http_build_query(["domain" => $domain, "account" => $key, "token" => $_SESSION["token"]]),
       ) ?>" title="<?= tohtml(_("Edit Mail Account")) ?>: <?= tohtml($key) ?>@<?= tohtml($domain) ?>">
								<?= tohtml($key . "@" . $domain) ?>
							</a>
						<?php } ?>
							...
							<?php if ($data[$key]["SUSPENDED"] != "yes") { ?>
								<li class="units-table-row-action" data-key-action="href">
									<a
										...
										href="http://<?= tohtml($v_webmail_alias) ?>.<?= tohtml($domain) ?>/?<?= tohtml(
	http_build_query(["_user" => $key . "@" . $domain]),
) ?>"
										target="_blank"
