<?php
$user_total = is_array($data) ? count($data) : 0;
$user_suspended = 0;
if (is_array($data)) {
...
$user_active = $user_total - $user_suspended;
?>
<!-- Begin toolbar -->
<div class="toolbar">
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			<a href="/add/user/" class="button js-button-create">
				<i class="fas fa-plus"></i><?= tohtml(_("Add User")) ?>
			</a>
			<a href="/list/package/" class="button button-secondary">
				<i class="fas fa-box-open"></i><?= tohtml(_("Packages")) ?>
			</a>
		</div>
		<div class="toolbar-right">
     	...
     	$_GET["q"] ?? "",
     ) ?>" placeholder="<?= tohtml(_("Filter users…")) ?>" title="<?= tohtml(_("Search")) ?>" autocomplete="off">
...
<div class="container" x-data="vzUserList">

	<div class="vz-page-hero">
		<div>
			<h1 class="vz-page-title"><?= tohtml(_("Users")) ?></h1>
			<p class="vz-page-subtitle"><?= tohtml(_("Manage user accounts, packages, and access.")) ?></p>
		</div>
		<div class="vz-stat-pills">
			<span class="vz-stat-pill"><strong><?= (int) $user_total ?></strong> <?= tohtml(_("users")) ?></span>
			<span class="vz-stat-pill is-success"><strong><?= (int) $user_active ?></strong> <?= tohtml(_("active")) ?></span>
			<?php if ($user_suspended > 0) { ?>
				<span class="vz-stat-pill is-danger"><strong><?= (int) $user_suspended ?></strong> <?= tohtml(_("suspended")) ?></span>
			<?php } ?>
		</div>
	</div>
  ...
  foreach ($data as $key => $value) {
 	...
 	echo "vz-filter-row";
				...
				data-name="<?= tohtml(strtolower($key . " " . ($data[$key]["NAME"] ?? "") . " " . ($data[$key]["CONTACT"] ?? ""))) ?>"
				<?php if (!$row_hidden) { ?>
					x-show="isVisible($el)"
	...
	_("Select"),
       	...
       	http_build_query(["user" => $key, "token" => $_SESSION["token"]]),
						...
						<?php } else { ?>
							<a href="/login/?<?= tohtml(
       	http_build_query(["loginas" => $key, "token" => $_SESSION["token"]]),
						...
						<?php if (!($key == $_SESSION["ROOT_USER"] || $key == $user_plain)) { ?>
							<li class="units-table-row-action shortcut-s" data-key-action="js">
								<a
									...
									href="/<?= tohtml($spnd_action) ?>/user/?<?= tohtml(
	http_build_query(["user" => $key, "token" => $_SESSION["token"]]),
) ?>"
        	...
        	http_build_query(["package" => $data[$key]["PACKAGE"], "token" => $_SESSION["token"]]),
				...
				<div class="units-table-cell u-text-center-desktop u-text-no-wrap">
					...
					<span class="u-text-bold">
						<?= tohtml(humanize_usage_size($data[$key]["DISK_QUOTA"], 1)) ?>
					</span>
					<span class="u-text-small">
				...
				<div class="units-table-cell u-text-center-desktop u-text-no-wrap">
					...
					<span class="u-text-bold">
						<?= tohtml(humanize_usage_size($data[$key]["BANDWIDTH"], 1)) ?>
					</span>
					<span class="u-text-small">
