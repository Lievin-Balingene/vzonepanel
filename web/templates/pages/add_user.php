...
<div class="container">
	...
	>
		...
		<div class="vz-page-hero u-mb20">
			<div>
				<h1 class="vz-page-title"><?= tohtml(_("Add User")) ?></h1>
				<p class="vz-page-subtitle"><?= tohtml(_("Create a new account with package, role, and login settings.")) ?></p>
			</div>
		</div>
		...
		<div class="form-container vz-form-card">
			...
			<div class="vz-form-section">
				...
				<div class="u-mb10">
					...
					<input type="text" class="form-control" name="v_username" id="v_username" value="<?= tohtml(
     	trim($v_username, "'"),
     ) ?>" tabindex="1" required autofocus>
				</div>
				<div class="u-mb10">
					<label for="v_name" class="form-label"><?= tohtml(_("Contact Name")) ?></label>
					<input type="text" class="form-control" name="v_name" id="v_name" value="<?= tohtml(
     	trim($v_name, "'"),
     ) ?>" tabindex="2" required>
				</div>
				<div class="u-mb10">
					<label for="v_email" class="form-label"><?= tohtml(_("Email")) ?></label>
					<input type="email" class="form-control js-sync-email-input" name="v_email" id="v_email" value="<?= tohtml(
     	trim($v_email, "'"),
     ) ?>" tabindex="3" required>
				</div>
				<div class="u-mb10">
					...
					<div class="u-pos-relative u-mb10">
						<input type="text" class="form-control js-password-input" name="v_password" id="v_password" value="<?= tohtml(
      	trim($v_password, "'"),
      ) ?>" tabindex="4" required>
			...
			<div class="vz-form-section">
				<h2 class="vz-form-section-title"><?= tohtml(_("Access & package")) ?></h2>
      	...
      	echo ">" . htmlentities($value) . "</option>\n";
				...
				<div class="u-mb10">
					...
					<select class="form-select" name="v_role" id="v_role" required>
						...
						<option value="admin" <?= tohtml($v_role == "admin" ? "selected" : "") ?>><?= tohtml(
	_("Administrator"),
) ?></option>
						<option value="dns-cluster" <?= tohtml($v_role == "dns-cluster" ? "selected" : "") ?>><?= tohtml(
	_("DNS Sync User"),
) ?></option>
					</select>
				</div>
      	...
      	echo ">" . htmlentities($key) . "</option>\n";
				...
				<div class="u-mb20">
					...
					<input type="email" class="form-control js-sync-email-output" name="v_notify" id="v_notify" value="<?= tohtml(
     	trim($v_notify, "'"),
     ) ?>" tabindex="8">
				</div>
			</div>
			...
			<div class="vz-form-actions">
				<button type="submit" class="button">
					<i class="fas fa-plus"></i><?= tohtml(_("Create user")) ?>
				</button>
				<a href="/list/user/" class="button button-secondary"><?= tohtml(_("Cancel")) ?></a>
			</div>
		</div>
