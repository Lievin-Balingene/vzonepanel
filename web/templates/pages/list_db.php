...
$db_total = is_array($data) ? count($data) : 0;
$db_suspended = 0;
$db_mysql = 0;
$db_pgsql = 0;
if (is_array($data)) {
...
$db_active = $db_total - $db_suspended;
?>
...
<div class="toolbar">
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			<?php if ($read_only !== "true") { ?>
				<a href="/add/db/" class="button js-button-create">
					<i class="fas fa-plus"></i><?= tohtml(_("Add Database")) ?>
				</a>
				<?php if (
    	...
    	$_SESSION["DB_SYSTEM"] === "pgsql,mysql"
					...
					<a class="button button-secondary <?= tohtml(ipUsed() ? "button-suspended" : "") ?>" href="<?= tohtml(
	$db_myadmin_link,
) ?>" target="_blank">
	...
	$db_pgadmin_link,
		...
		<div class="toolbar-right">
     	...
     	$_POST["q"] ?? "",
     ) ?>" placeholder="<?= tohtml(_("Filter databases…")) ?>" title="<?= tohtml(_("Search")) ?>" autocomplete="off">
...
<div class="container" x-data="vzDbList">

	<div class="vz-page-hero">
		<div>
			<h1 class="vz-page-title"><?= tohtml(_("Databases")) ?></h1>
			<p class="vz-page-subtitle"><?= tohtml(_("Manage MySQL and PostgreSQL databases.")) ?></p>
		</div>
		<div class="vz-stat-pills">
			<span class="vz-stat-pill"><strong><?= (int) $db_total ?></strong> <?= tohtml(_("total")) ?></span>
			<span class="vz-stat-pill is-success"><strong><?= (int) $db_active ?></strong> <?= tohtml(_("active")) ?></span>
			<span class="vz-stat-pill is-info"><strong><?= (int) $db_mysql ?></strong> MySQL</span>
			<span class="vz-stat-pill is-info"><strong><?= (int) $db_pgsql ?></strong> PostgreSQL</span>
			<?php if ($db_suspended > 0) { ?>
				<span class="vz-stat-pill is-danger"><strong><?= (int) $db_suspended ?></strong> <?= tohtml(_("suspended")) ?></span>
			<?php } ?>
		</div>
	</div>
	...
	<div class="vz-filter-bar">
		...
		<button type="button" class="vz-chip" :class="filter === 'mysql' && 'active'" @click="filter = 'mysql'">
			MySQL
		</button>
		<button type="button" class="vz-chip" :class="filter === 'pgsql' && 'active'" @click="filter = 'pgsql'">
			PostgreSQL
		</button>
  ...
  foreach ($data as $key => $value) {
  	...
  	if ($data[$key]["TYPE"] == "mysql" && !empty($_SESSION["DB_PMA_ALIAS"])) {
  		$db_admin_link = $_SESSION["DB_PMA_ALIAS"];
  	}
  	...
  	if ($data[$key]["TYPE"] == "pgsql" && !empty($_SESSION["DB_PGA_ALIAS"])) {
	...
	_("Select"),
) ?>" name="database[]" value="<?= tohtml($key) ?>" <?= tohtml($display_mode) ?>>
				...
				<div class="units-table-cell units-table-heading-cell u-text-bold">
					...
					<div class="vz-web-name">
						<?php if ($read_only === "true" || $data[$key]["SUSPENDED"] == "yes") { ?>
							<?= tohtml($key) ?>
						<?php } else { ?>
							<a href="/edit/db/?<?= tohtml(
        	http_build_query(["database" => $key, "token" => $_SESSION["token"]]),
        ) ?>" title="<?= tohtml(_("Edit Database")) ?>: <?= tohtml($key) ?>">
								<?= tohtml($key) ?>
							</a>
						<?php } ?>
         	...
         	http_build_query(["database" => $key, "token" => $_SESSION["token"]]),
							...
							<li class="units-table-row-action shortcut-s" data-key-action="js">
								<a
									...
									href="/<?= tohtml($spnd_action) ?>/db/?<?= tohtml(
	http_build_query(["database" => $key, "token" => $_SESSION["token"]]),
) ?>"
