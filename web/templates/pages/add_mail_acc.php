...
<div class="toolbar">
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			<a class="button button-secondary button-back js-button-back" href="/list/mail/?<?= tohtml(
   	http_build_query(["domain" => trim($v_domain, "'"), "token" => $_SESSION["token"]]),
   ) ?>">
...
<div class="container">
	...
	>
		...
		<div class="vz-page-hero u-mb20">
			<div>
				<h1 class="vz-page-title"><?= tohtml(_("Add Mail Account")) ?></h1>
				<p class="vz-page-subtitle"><?= tohtml(
    	sprintf(_("Create a mailbox on %s"), trim($v_domain, "'")),
    ) ?></p>
			</div>
		</div>
		...
		<div class="form-container form-container-wide vz-form-card">
			...
			<div class="sidebar-right-grid">
				<div class="sidebar-right-grid-content">
					<div class="vz-form-section">
						<h2 class="vz-form-section-title"><?= tohtml(_("Account details")) ?></h2>
						<div class="u-mb10">
       	...
       	trim($v_domain, "'"),
						...
						<div class="u-mb10">
							...
							<input type="text" class="form-control js-account-input" name="v_account" id="v_account" value="<?= tohtml(
       	trim($v_account, "'"),
       ) ?>" required>
						</div>
						<div class="u-mb10">
							<label for="v_password" class="form-label">
								<?= tohtml(_("Password")) ?>
								<button type="button" title="<?= tohtml(
        	_("Generate"),
						...
						<div class="vz-form-section">
							...
							<div class="u-mb10">
								...
								<div class="u-pos-relative">
									<input type="text" class="form-control" name="v_quota" id="v_quota" value="<?= tohtml(
         	trim($v_quota, "'"),
         ) ?>">
							...
							<div class="u-mb10">
								<label for="v_aliases" class="form-label">
									<?= tohtml(_("Aliases")) ?> <span class="optional">(<?= tohtml(
 	_("Use local-part without domain name"),
 ) ?>)</span>
								</label>
								<textarea class="form-control" name="v_aliases" id="v_aliases"><?= tohtml(
        	trim($v_aliases, "'"),
        ) ?></textarea>
							</div>
							<div class="u-mb10">
								<label for="v_fwd" class="form-label">
									<?= tohtml(_("Forward to")) ?> <span class="optional">(<?= tohtml(
 	_("One or more email addresses"),
 ) ?>)</span>
								</label>
								<textarea class="form-control js-forward-to-textarea" name="v_fwd" id="v_fwd" <?php if (
        	$v_blackhole == "yes"
        ) {
        	echo "disabled";
							...
							<div class="form-check">
								<input class="form-check-input js-discard-all-mail" type="checkbox" name="v_blackhole" id="v_blackhole" <?php if (
        	$v_blackhole == "yes"
        ) {
        	echo "checked";
        } ?>>
        	...
        	echo "u-hidden";
								...
								<input class="form-check-input js-do-not-store-checkbox" type="checkbox" name="v_fwd_only" id="v_fwd_for" <?php if (
        	$v_fwd_only == "yes"
        ) {
        	echo "checked";
        } ?>>
							...
							<div class="u-mt10 u-mb10">
								...
								<input type="text" class="form-control" name="v_rate" id="v_rate" value="<?= tohtml(
        	trim($v_rate, "'"),
 	...
 	echo "disabled";
					...
					<div class="u-mt15 u-mb20">
						...
						<input type="email" class="form-control" name="v_send_email" id="v_send_email" value="<?= tohtml(
      	trim($v_send_email, "'"),
      ) ?>">
					</div>
				</div>
