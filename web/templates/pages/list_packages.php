<?php
$package_total = is_array($data) ? count($data) : 0;
?>
<!-- Begin toolbar -->
<div class="toolbar">
	<div class="toolbar-inner">
		<div class="toolbar-buttons">
			...
			<a href="/add/package/" class="button js-button-create">
				<i class="fas fa-plus"></i><?= tohtml(_("Add Package")) ?>
			</a>
		</div>
		<div class="toolbar-right">
     	...
     	$_GET["q"] ?? "",
     ) ?>" placeholder="<?= tohtml(_("Filter packages…")) ?>" title="<?= tohtml(_("Search")) ?>" autocomplete="off">
...
<div class="container" x-data="vzPackageList">

	<div class="vz-page-hero">
		<div>
			<h1 class="vz-page-title"><?= tohtml(_("Packages")) ?></h1>
			<p class="vz-page-subtitle"><?= tohtml(_("Define resource limits and feature sets for users.")) ?></p>
		</div>
		<div class="vz-stat-pills">
			<span class="vz-stat-pill"><strong><?= (int) $package_total ?></strong> <?= tohtml(_("packages")) ?></span>
		</div>
	</div>
  ...
  foreach ($data as $key => $value) {
				...
				<div class="units-table-cell units-table-heading-cell u-text-bold">
					...
					<div class="vz-web-name">
						<?php if ($key == "system") { ?>
							<?= tohtml($key) ?>
						<?php } else { ?>
							<a href="/edit/package/?<?= tohtml(
        	http_build_query(["package" => $key, "token" => $_SESSION["token"]]),
        ) ?>" title="<?= tohtml(_("Edit Package")) ?>: <?= tohtml($key) ?>">
								<?= tohtml($key) ?>
							</a>
						<?php } ?>
						<div class="vz-web-badges">
							<span class="vz-badge vz-badge-success"><?= tohtml(_("Active")) ?></span>
						</div>
					</div>
				</div>
				<div class="units-table-cell">
				...
				<div class="units-table-cell u-text-center-desktop">
					...
					<?php if ($data[$key]["SHELL"] == "nologin") { ?>
						<i class="fas fa-circle-minus icon-large" title="<?= tohtml(_("SSH Access")) ?>: <?= tohtml(
	$data[$key]["SHELL"],
) ?>"> </i>
					<?php } else { ?>
				...
				<div class="units-table-cell u-text-center-desktop">
					...
					<span title="<?= tohtml(_("Quota")) ?>: <?= tohtml(
	humanize_usage_size($data[$key]["DISK_QUOTA"]),
				...
				<div class="units-table-cell u-text-center-desktop">
					...
					<span title="<?= tohtml(_("Bandwidth")) ?>: <?= tohtml(
	humanize_usage_size($data[$key]["BANDWIDTH"]),
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("Web Domains")) ?>: <?= tohtml(
	$data[$key]["WEB_DOMAINS"],
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("Web Aliases")) ?>: <?= tohtml(
	$data[$key]["WEB_ALIASES"],
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("DNS Zones")) ?>: <?= tohtml(
	$data[$key]["DNS_DOMAINS"],
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("DNS Records")) ?>: <?= tohtml(
	$data[$key]["DNS_RECORDS"],
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("Mail Domains")) ?>: <?= tohtml(
	$data[$key]["MAIL_DOMAINS"],
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("Mail Accounts")) ?>: <?= tohtml(
	$data[$key]["MAIL_ACCOUNTS"],
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("Databases")) ?>: <?= tohtml(
	$data[$key]["DATABASES"],
) ?>">
				...
				<div class="units-table-cell compact u-text-bold u-text-center-desktop">
					...
					<span class="units-table-badge" title="<?= tohtml(_("Cron Jobs")) ?>: <?= tohtml(
	$data[$key]["CRON_JOBS"],
) ?>">
